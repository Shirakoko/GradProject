...
class SleevePanel(pyg.Panel):
    ...
    def __init__(self, name, body, design, open_shape):
        ...
        rest_angle = max(np.deg2rad(design['sleeve_angle']['v']), shoulder_angle) # rest_angle取sleeve_angle和shoulder_angle中的较大值
        ...
        arm_width = abs(open_shape[0].start[1] - open_shape[-1].end[1])  # 袖笼弧线宽度
        end_width = design['end_width']['v'] * arm_width # 底边长度
