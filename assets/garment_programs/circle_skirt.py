...
class CircleArcPanel(pyg.Panel):
    ...
    def __init__(self, name, top_rad, length, angle) -> None:
        ...
        dist_w = 2 * top_rad * np.sin(halfarc) # 上圆弧的宽度
        dist_out = 2 * (top_rad + length) * np.sin(halfarc) # 下圆弧的宽度

        vert_len = length * np.cos(halfarc) # 垂直于两圆弧切线的长度
