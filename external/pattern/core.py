...
import os
import random
from converts_form import convert_form_from_str
...
class BasicPattern(object):
    ...
    def serialize(self, path, to_subfolder=True, tag='', empty_ok=False):
        ...
        # log context
        if to_subfolder:
            log_dir = os.path.join(path, self.name)  # NOTE Added change
            try:
                os.makedirs(log_dir)
            except OSError as e:
                if e.errno != errno.EEXIST:
                    raise
            spec_file = os.path.join(log_dir, 'panels.json') # 输出panels.json
        else:
            log_dir = path
            spec_file = os.path.join(path, (self.name + tag + '_specification.json'))

        # 把garmentcode的json格式转换成我们需要的panels.json的格式并保存到指定路径
        convert_form_from_str(self.spec, spec_file)
        # Save specification
        # with open(spec_file, 'w') as f_json:
        #     json.dump(self.spec, f_json, indent=2) # 把self.spec写入json文件，缩进2格
