...
class SkirtPanel(pyg.Panel):
    ...
    def __init__(self, name, waist_length=70, length=70, ruffles=1, bottom_cut=0, flare=0) -> None:
        ...
        base_width = waist_length / 2 # 横向基本宽度
        top_width = base_width * ruffles # 梯形上边宽度=横向基本宽度*褶皱倍数
        low_width = top_width + 2*flare # 梯形下边宽度=梯形上边宽度+2*单侧延伸长度
        x_shift_top = (low_width - top_width) / 2  # to account for flare at the bottom # 这不就是flare吗？？？

        # 定义边
        self.right = pyg.esf.side_with_cut([0,0], [x_shift_top, length], start_cut=bottom_cut) if bottom_cut else pyg.EdgeSequence(pyg.Edge([0,0], [x_shift_top, length]))
        self.waist = pyg.Edge(self.right[-1].end, [x_shift_top + top_width, length])
        self.left = pyg.esf.side_with_cut(self.waist.end, [low_width, 0], end_cut=bottom_cut) if bottom_cut else pyg.EdgeSequence(pyg.Edge(self.waist.end, [low_width, 0]))
...
class PencilSkirt(pyg.Component):
    def __init__(self, body, design) -> None:
        ...
        self.length = design['length']['v'] * body['leg_length']
        ...
        self.front = FittedSkirtPanel( # 包臀裙前片
            f'skirt_f',   
            waist= body['waist'] / 4,
            hips = body['hips'] / 4,
            hips_depth = body['hips_line'],
            length = self.length,
            low_width=design['flare']['v'] * body['hips'] / 4, # 下摆宽度 = 1/4 * 臀宽（最粗的地方） * 缩放比例，如果flare = 1说明是直筒
            rise=design['rise']['v'],
            low_angle=design['low_angle']['v'],
            dart_position=body['bust_points'] / 2,
            dart_frac=design['front_dart']['v'] + 1,  # 包臀裙前片的省道深度（百分比）
            cut=design['front_cut']['v'], 
            side_cut=style_shape
        ).translate_to([0, body['waist_level'], 25])

        self.back = FittedSkirtPanel( # 包臀裙后片
            f'skirt_b', 
            waist = body['waist'] / 4, 
            hips = body['hips'] / 4,
            hips_depth = body['hips_line'],
            ...
            dart_frac=design['back_dart']['v']+1, # 包臀裙后片的省道深度（百分比）
